  ...
  ITxn *txn, RC &rc, size_t &cnt) {
    ...
    for (auto& req : fetch_req) {
      ...
      // auto tuples = ((ObjectBufferManager *) g_buf_mgr)->LoadRangeFromStorage(
          // GetTableId(), low, high, result_cnt);
      // // auto range_sz = high.ToUInt64() - low.ToUInt64() + 1;
      // LOG_DEBUG("remote fetch req: %lu - %lu size = %lu", low.ToUInt64(), high.ToUInt64(), result_cnt);
      ...
      //   //hack: need to remove this line
      //   tags[key.ToUInt64() - low_key.ToUInt64()].Init(tuple);
      //   // // LOG_DEBUG("add remote tuple %lu to range search results", key.ToUInt64());
      //   // rc = index->Insert(key, tuple, AccessType::READ, nullptr, tag);
      //   // if ((i < result_cnt - 1) && tag->Get() != tuple) {
      //   //   auto existed_tuple = reinterpret_cast<ITuple*>(tag->Get());
      //   //   existed_tuple->UnsetGapBit();
      ...
      //   char * data;
      //   size_t sz;
      //   rc = txn->AccessTuple(&tags[key.ToUInt64() - low_key.ToUInt64()], this, key.ToUInt64(),
      //                    SCAN, data, sz);
      //   if (rc == RC::ABORT) {
      //     LOG_DEBUG("Scan abort for Gap %d (low key %d, high key %d)", key.ToUInt64(), low_key.ToUInt64(), high_key.ToUInt64());
      //     break;
      //   }
      // }
      // DEALLOC(tuples);
  ...
  ITxn *txn, RC &rc, size_t &cnt) {
    ...
    for (auto& req : fetch_req) {
      ...
      M_ASSERT((low.ToUInt64() >= low_key.ToUInt64()) && (high.ToUInt64() <= high_key.ToUInt64() && low.ToUInt64() <= high.ToUInt64()),
               "unexpected remote fetch(low %ld, high %ld), original range query with (low key %ld, high key %ld)",
               low.ToUInt64(), high.ToUInt64(), low_key.ToUInt64(), high_key.ToUInt64());
      // LOG_INFO("Cache Miss, to remote fetch req: %lu - %lu, triggered by original range query (low key %ld, high key %ld)",
      //          low.ToUInt64(), high.ToUInt64(), low_key.ToUInt64(), high_key.ToUInt64());
      // std::cout.flush();
    ...
    if (g_enable_partition_covering_lock) {
      ...
      for (int i = start_pid; i <= end_pid; i++) {
          ...
          if (rc == ABORT) {
            ...
            // }
            LOG_INFO("XXX Failed to Acq S intention lock on pid %d for low key %ld", i, low_key.ToUInt64());
            return nullptr;
          }
          // LOG_INFO("XXX Success to Acq S lock on pid %d", i);
