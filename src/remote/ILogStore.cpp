...
//      totalLength += data_[i].length();
//   }

//   // for (const std::string& str : data_) {
...
//   for (size_t i = 0; i < sz; i++) {
//       auto str = data_[i];
//       std::strcpy(result + currentIndex, str.c_str());
...
//   // flush log
// #if REMOTE_STORAGE_TYPE == REMOTE_STORAGE_AZURE_TABLE
//   // TODO(Hippo): recover log store
//   // void StoreSync(const std::string &tbl_name, std::string partition, const std::string &key, char * data, size_t sz) {
//   client_->StoreSync(storage_id_, std::to_string(flush_lsn_ % 1000), std::to_string(cnt_), result, totalLength);
