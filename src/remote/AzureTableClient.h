...
// XXX TODO
  RC LoadSync(const std::string& tbl_name, std::string partition, const std::string& key, std::string& data) {
    // if (partition.find("metadata") != std::string::npos) {
      ...
      while (retry_left >= 0) {
        try {
          ...
          if (!entity.etag().empty()) {
            ...
            LOG_DEBUG("entity not found for key %s in parition %s in table %s", key.c_str(), partition.c_str(),
            tbl_name.c_str());
            std::cout.flush();
            retry_left = -1;
            return RC::NOT_FOUND;
  ...
  void StoreSync(const std::string &tbl_name, std::string partition, const std::string &key, char * data, size_t sz) {
    ...
    while (should_retry) {
     try {
      auto result = table.execute(insert_op);
      // if (tbl_name.find("obj-10g-0") != std::string::npos) {
      //      LOG_DEBUG("StoreSync for parition: %s, key: %s, retry: %d, table: %s", partition.c_str(), key.c_str(), retry, tbl_name.c_str());
      // } 
      // if (partition.find("metadata") != std::string::npos) {
      //     auto result = table.execute(insert_op);
      //     LOG_DEBUG("StoreSync for parition: %s, key: %s, retry: %d, table: %s", partition.c_str(), key.c_str(), retry, tbl_name.c_str());
      // } else if (tbl_name.find("page-wl250-10g-268435456") != std::string::npos) {
      //      LOG_DEBUG("StoreSync for parition: %s, key: %s, retry: %d, table: %s", partition.c_str(), key.c_str(), retry, tbl_name.c_str());
      // } 
      should_retry = false;
     } catch (const std::exception &e) {
  ...
  void WriteBatchAsync(const std::string &tbl_name, std::string partition, const std::multimap<std::string, std::string> &data, std::function<void()> callback) {
    // LOG_INFO("Start WriteBatchAsync for able_name: %s, partition name: %s", tbl_name.c_str(), partition.c_str());
    // std::cout.flush();
    // auto it = tables_.find(tbl_name);
    // if (it == tables_.end()) {
    //   M_ASSERT(false, "WriteBatchAsync encounter exception: table not found");
    // }
    auto table = tables_[tbl_name];
    azure::storage::table_batch_operation batch_op;
    ...
    try {
      // auto resp = table.execute_batch(batch_op);
      ...
      // resp.then([callback](const std::vector<azure::storage::table_result>& results) {
      //       LOG_DEBUG("One WriteBatchAsync success");
      //       std::cout.flush();
      //       callback();
      //     })
      //     .then([callback](pplx::task<void> t) {
      //       // Handle exceptions from async operation
      //       try {
      //         t.get();  // Will throw if task encountered an exception
      //         LOG_DEBUG("One WriteBatchAsync success");
      //         std::cout.flush();
      //         callback();
      //       } catch (const std::exception& e) {
  ...
  //                      std::string low_key, std::string high_key, char **data_ptrs, size_t& res_cnt) {
  //   // async execution.
  //   python_helper_->ExecPyRangeSearch(tbl_name, partition,
  //                                     low_key, high_key, data_ptrs, res_cnt);
  // }
