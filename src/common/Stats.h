...
// x(log_group_sz_, 0) x(num_flushes_, 0) x(log_group_payload_sz_, 0)
// #define COMMIT_INT_STATS(x) x(commit_cnt_, 0) x(num_logs_, 0) \
// x(commit_queue_sz_, 0) x(num_commit_queue_samples_, 0) x(txn_cnt_cmt_, 0) x(scan_txn_cnt_cmt_, 0) x(rw_txn_cnt_cmt_, 0)       \
// x(log_group_sz_, 0) x(num_flushes_, 0) x(log_group_payload_sz_, 0)
...
// #define COMMIT_TIME_STATS(x) x(log_latency_, 0) x(log_flush_interval_, 0) x(abort_time_cmt_, 0) x(commit_time_cmt_, 0) x(scan_abort_time_cmt_, 0) x(rw_abort_time_cmt_, 0) x(cc_time_cmt_, 0)
// #define COMMIT_VEC_STATS(x) x(commit_latency_, 0) x(log_flush_latency_, 0) x(txn_latency_cmt_, 0) x(rw_txn_latency_cmt_, 0) x(scan_txn_latency_cmt_, 0) 
...
class Stats {
 public:
  ...
  void Print() {
    ...
    // PRINT_AVG_US_STATS(sum_commit_stats_, sum_txn_latency_cmt, _us, txn_cnt_cmt_, 1)
    // PRINT_AVG_US_STATS(sum_commit_stats_, sum_scan_txn_latency_cmt, _us, scan_txn_cnt_cmt_, 1)
    // PRINT_AVG_US_STATS(sum_commit_stats_, sum_rw_txn_latency_cmt, _us, rw_txn_cnt_cmt_, 1)

    // PRINT_AVG_US_STATS(sum_commit_stats_, abort_time_cmt_, ms_, txn_cnt_cmt_, 1000)
    // PRINT_AVG_US_STATS(sum_commit_stats_, scan_abort_time_cmt_, ms_, scan_txn_cnt_cmt_, 1000)
    // PRINT_AVG_US_STATS(sum_commit_stats_, rw_abort_time_cmt_, ms_, rw_txn_cnt_cmt_, 1000)
    // PRINT_AVG_US_STATS(sum_commit_stats_, cc_time_cmt_, ms_, txn_cnt_cmt_, 1000)
