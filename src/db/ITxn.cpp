...
high_key) {
  ...
  for (size_t i = 0; i < cnt; i++) {
    ...
    // TODO: validate the result correctness
    // if (!result[i].Get())
...
RC ITxn::GetTuple(ITable *p_table, OID idx_id, SearchKey key, AccessType ac_type,
  ...
  if (g_enable_partition_covering_lock && ac_type == AccessType::UPDATE) {
      ...
      if (rc == ABORT) {
        ...
        // LOG_INFO("XXX Failed to Acq IX lock on pid %d", pid);
        return rc;
      }
      // LOG_INFO("XXX Success to Acq IX lock on pid %d", pid);
...
RC ITxn::AccessTuple(SharedPtr *result, ITable * p_table, uint64_t expected_key,
                     AccessType ac_type, char *&data, size_t &sz, bool free_ptr) {
  ...
  //TODO(Hippo): enable correctness check for scan scenarios
  ...
  if (need_lock) {
     ...
     if (rc == ABORT) {
        // LOG_INFO("XXX Failed to Acq X lock on tuples in pid %d for key %ld", GetPartitionID(key.ToUInt64()), key.ToUInt64());
...
void ITxn::Abort() {
  ...
  if (g_warmup_finished) {
    ...
    LOG_INFO("XXX abort count is %ld is for worker id %ld", g_stats->db_stats_[Worker::GetThdId()].abort_cnt_, Worker::GetThdId());
  ...
  for (auto & iac: intention_locks_ ) {
    //  LOG_INFO("XXX release %s lock for pid %d in abort", (iac.lock_type == S)? "S":"IX", iac.partition_id_)
...
void ITxn::PreCommit() {
  ...
  for (auto & iac: intention_locks_) {
    ...
    //  LOG_INFO("XXX release %s lock for pid %d in precommit", (iac.lock_type == ILockType::S)? "S":"IX", iac.partition_id_)
     CCManager::ReleaseIntentionLock(*(iac.lock_), iac.lock_type);
  }
...
RC ITxn::Commit() {
  ...
  //  else {
  //   LOG_INFO("XXX Read only, no Commit log");
  // }
  flushed_ = flushed;
  ...
  for (auto & iac: intention_locks_) {
     //TODO(Hippo): confirm how to deal with intention lock for g_early_lock_release
     if ((!g_early_lock_release) || (iac.lock_type == ILockType::IX)) {
        // LOG_INFO("XXX release %s lock for pid %d in commit", (iac.lock_type == ILockType::S)? "S":"IX", iac.partition_id_)
        CCManager::ReleaseIntentionLock(*(iac.lock_), iac.lock_type);
     }
  }
  ...
  if (flushed && g_warmup_finished && !g_terminate_exec) {
    ...
    // if (is_read_only) {
    //   LOG_INFO("Read log latency %.2f us, read txn latency %.2f us", log_time_*1.0/1000, latency*1.0/1000);
    // }
