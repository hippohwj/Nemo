...
RC IndexBTreeObject::RangeEntryIter(SearchKey key, ITuple* entry, SearchKey start, SearchKey end,  std::vector<std::pair<SearchKey, SearchKey>>& fetch_req,  ITxn* txn, IdxAccess& idx_ac) {
  ...
  if (rc != RC::OK) {
    ...
    LOG_INFO("RangeEntryIter current rc is %s", RCToString(rc).c_str());
    return rc;
  } else {
...
SharedPtr* IndexBTreeObject::RangeSearch(SearchKey start, SearchKey end, ITxn* txn,
  ...
  //   M_ASSERT(false, "support phantom protection for range query");
  // }

  // TODO: after enable phantom protection, need to set the default rc as ERROR
  //  rc = RC::ERROR;
  rc = RC::OK;

  // auto result = NEW_SZ(SharedPtr, end.ToUInt64() - start.ToUInt64() + 100);
  // for (size_t i = 0; i < end.ToUInt64() - start.ToUInt64() + 100; i++) {
  ...
  // iterate till reaching the largest key that is <= end
  while (rc == RC::OK) {
...
// SharedPtr* IndexBTreeObject::RangeSearch(SearchKey start, SearchKey end, ITxn* txn,
//                                          std::vector<std::pair<SearchKey, SearchKey>>& fetch_req, RC& rc, size_t& cnt) {
//   // RC::ERROR - key not found
//   // RC::OK - key is found
//   // RC::ABORT - lock conflict with insert operations.
//   // TODO: here we assume pkey (discrete values) so that we can preallocate space
...
//   // find the largest entry that has a key less than or equal to the start key
//   M_ASSERT(child->right_bound_.ToUInt64() >= start.ToUInt64(), "right bound %ld should > start %ld", child->right_bound_.ToUInt64(), start.ToUInt64());
//   M_ASSERT(child->left_bound_.ToUInt64() <= start.ToUInt64(), "left bound %ld should <= start %ld", child->left_bound_.ToUInt64(), start.ToUInt64());

//   SearchKey last_key = start;
//   bool last_exist = true;
//   bool stop = false;

//   // the first index (first_item_idx) of the cached items that satisfy start < child->keys[first_item_idx]
//   size_t first_item_idx = 0;

//   // check item idx 0 separately because it may be the first_item_idx and if so the last key and last_exist should be
//   // acuqired from a prior leaf node
//   if (start < child->keys_[0]) {
//     // check the case if start < child->keys[0]:  range [start, child->keys_[0])
//     if (child->prev_ != nullptr) {
//       // find the last leaf node that has a key less than or equal to the start key
//       auto prev = const_cast<BTreeObjNode*>(child->prev_);
//       prev->latch_.LockSH();
//       // M_ASSERT(prev->key_cnt_ > 0, "the previous leaf node should have at least one key");
//       // TODO: investigate why prev node has key_cnt_ == 0
//       if (prev->key_cnt_ <= 0) {
//         M_ASSERT(false, "each leaf node should have at least one key");
//         last_exist = true;
//       } else {
//         last_exist = reinterpret_cast<ITuple*>(prev->children_[prev->key_cnt_].Get())->IsGapBitSet();
//       }
//       // last_exist = prev->children_[prev->key_cnt_].Get()->IsGapBitSet();
//       prev->latch_.Unlock();
//       last_key = start;
//     } else {
//       // no prior leaf exist
...
//     }
//   } else {
//     // Otherwise, find the first index first_item_idx that start < child->keys[first_item_idx]
//     // (in other words, find the first index i that key[i] falls in [start, end])
//     bool found = false;
//     for (int i = 0; i < child->key_cnt_; i++) {
//       if (start < child->keys_[i]) {
//         // this branch: start < child->keys_[i] && i > 0
//         first_item_idx = i;
//         last_exist = reinterpret_cast<ITuple*>(child->children_[i + 1].Get())->IsGapBitSet();
//         last_key = start;
//         found = true;
//         break;
//       } else {
...
//         }
//         last_exist = reinterpret_cast<ITuple*>(child->children_[i + 1].Get())->IsGapBitSet();
//         // last_key = child->keys_[i];
//         last_key = start;
//       }
//     }

//     if (!found) {
//       // if not found, it means that the start key is larger than all keys in the current leaf node
//       //  and the first_item_idx should be the first item of the next leaf node
//       if (child->next_ != nullptr) {
//         auto next = const_cast<BTreeObjNode*>(child->next_);
//         next->latch_.LockSH();
...
//         first_item_idx = 0;
//       } else {
//         // already reach the end of the leaf nodes
//         if (last_exist) {
//           fetch_req.emplace_back(last_key, end);
//           stop = true;
//         } else {
//           // no keys in the database fall in [start, end]
//           fetch_req.emplace_back(start, end);
//           stop = true;
//         }
//       }
//     }
...
//   // check every entry's gap bit if it falls in [start, end]
//   while (!stop) {
//     for (int i = first_item_idx; i < child->key_cnt_; i++) {
//       if (end <= child->keys_[i]) {
//         // find the end boundary, stop searching
//         if (last_exist) {
//           fetch_req.emplace_back(last_key, end);
//           stop = true;
//           break;
...
//       } else {
//         if (last_exist) {
//           //  fetch_req.emplace_back(last_key, child->keys_[i]);
//           fetch_req.emplace_back(last_key, end);
//           stop = true;
//           break;
//         } else {
...
//     // M_ASSERT(cnt > 0 || !fetch_req.empty(), "if not abort, result should not be empty.");
//     // LOG_INFO("Range query read cnt rows = %ld, fetch_req size = %ld", cnt, fetch_req.size());
//     if (g_enable_phantom_protection) txn->idx_accesses_.push_back(idx_ac);
//   }
...
//           child->keys_[i + 1];
//       // XXX: judge in next key in storage based on continuity;
...
//     for (size_t i = 0; i < end.ToUInt64() - start.ToUInt64() + 1; i++) {
//       if (result[i].Get() && 
//           reinterpret_cast<ITuple *>(result[i].Get())->GetTID() != start.ToUInt64() + i) {
//         LOG_ERROR("RangeSearch fetch error data, item index:%lld, size:%lld", i, end.ToUInt64() - start.ToUInt64() + 1); 
//       }
//     }
//   }
...
SharedPtr* IndexBTreeObject::Search(SearchKey key, AccessType ac_type, ITxn *txn, RC &rc) {
  ...
  //   LOG_INFO("exclusive phantom protection search key %ld", key.ToUInt64());
  // }
    ...
    if (child->keys_[i] == key) {
      ...
      if (is_exclusive_phantom_protection) {
        ...
        // TODO: Deal with node bounds (get prev_tuple for left bound)
           LOG_INFO("prev_tuple is null, target key %ld", key.ToUInt64());
           acq_rc = RC::ABORT;
  ...
  if (acq_rc == RC::ABORT) {
    rc = RC::ABORT;
    LOG_INFO("XXX search set status to abort");
    // release previously held index locks
    for (auto & lock : idx_ac.locks_) {
...
//       // LOG_INFO("Key to delete (%s) does not exist in leaf", key.ToString().c_str());
//       return false;
//     }
...
bool IndexBTreeObject::EvictRandomLeafNode() {
  ...
  if (evict_tuple_num == 0) {
    ...
    if (!DeleteSafe(child, evict_tuple_num) && !DeleteSafe(parent)) {
      child->latch_.Unlock();
      parent->latch_.Unlock();
      // TODO: be mindful that FindEvictionCandidates already clean the usage bit.
      // LOG_DEBUG("Eviction may incur spliting more than 2 levels, start pessimistic search!");
      // // PrintTree();
      // auto stack = PessimisticSearch(keys[0], parent, child, DELETE);
      // // find the candidates again
      // FindEvictionCandidates(child, keys, tuples);
      // // insert bottom-up recursively with a stack.
    ...
    child->latch_.Unlock();
    // LOG_INFO("Index Tree After a eviction:");
    // PrintTree();
    // std::cout.flush();
    return true;
  }
