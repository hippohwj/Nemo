...
void PageBufferManager::FlushAndLoad(OID buf_id, PageTag &tag, bool load_from_storage) {
  ...
  if (load_from_storage) {
    ...
    // if (buf_id == 0) {
      // auto store_key = pg.header_.tag.ToString();
      // LOG_DEBUG("XXX evict bufid %d, is dirty %s, tbl_name %s, pageid %d, key is %s", buf_id, desc->CheckFlag(BM_DIRTY)?"true":"false", ITable::GetStorageId(pg.header_.tag.first_).c_str(), pg.header_.tag.pg_id_,store_key.c_str());
    // }
...
void PageBufferManager::FlushAll() {
  ...
  while (allocated_ > 0 && evict_clock_hand_) {
    ...
    // if (table_name.find("page-wl250-10g-268435456") != std::string::npos) {
    //    LOG_DEBUG("XXX Write to table %s, with tblid: %d, pageid: %d", table_name.c_str(), pg.header_.tag.first_, pg.header_.tag.pg_id_);
    // }
    // g_data_store->Write(table_name, key, start, sizeof(BasePage));
    ClockRemoveHand(evict_clock_hand_, evict_clock_tail_, allocated_);
  }
