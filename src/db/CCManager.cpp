...
RC CCManager::AcquirePhantomIntentionLock(std::atomic<uint32_t> &state, bool is_exclusive) {
  ...
  if (lock_type == ILockType::IX) {
    ...
    while (true) {
      ...
      // If IX is held, mark S-waiting on THIS lock and return ABORT (to enforce priority).
...
//         if (((current << 16) >> 16) > 0) {  // Check if S lock is held
//           // continue;
//           // current = state.load(std::memory_order_acquire);
//           return ABORT;
//         } else {
//           uint32_t ix_locked = current + (((uint32_t)1) << 16);
...
//             if ((current >> 16) > 0) {  // Check if IX lock is held
//                 return ABORT;
//             }
            
//             uint32_t s_lock_count = (current << 16) >> 16;
//             // if (wait_state.load(std::memory_order_acquire) > 0) {
//             //     // yield to avoid starvation of IX locks
//             //     std::this_thread::yield();
//             //     current = state.load(std::memory_order_acquire);
//             //     continue;  // Recheck condition after yielding
//             // }

//             uint32_t s_locked = current + 1;
...
//   ILockType lock_type = is_exclusive ? ILockType::IX : ILockType::S;
// if (lock_type == ILockType::IX) {
//   auto prev = state.fetch_sub(((uint64_t)1) << 16);
//   M_ASSERT(prev >= ( 1<<16 ), "Releasing a IX lock that is not held");
// } else if (lock_type == ILockType::S) {
//   auto prev = state.fetch_sub(1);
//   M_ASSERT((prev << 16 >> 16)> 0, "Releasing a S lock that is not held");
// } else {
