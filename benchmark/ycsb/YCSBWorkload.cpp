...
void YCSBWorkload::Execute(YCSBWorkload *workload, BenchWorker *worker) {
  ...
  while (!g_terminate_exec) {
    if (rc != ABORT) {
      ...
      if (g_scan_workload_rw_thd_num == 0) {
    ...
    if (rc != RC::OK) {
      ...
      LOG_INFO("XXX backoff for abort with %d ms for retry %d with rc %s", sleep_ms_int, retry, RCToString(rc).c_str());
      if (tpe == YCSB_SCAN) {
        LOG_INFO("XXX backoff for scan abort");
      } else if (tpe == YCSB_INSERT) {
        LOG_INFO("XXX backoff for insert abort");
      }
...
void YCSBWorkload::WarmupNegativeScanGapbitPhantom() {
  ...
  // while (allocated < 0.9 * num_slots && iter < config_->domain_size_) {
  // while (allocated < 0.9 * num_slots) {
  while (allocated < num_slots && iter < config_->warmup_scan_max_iter_) {
  // while (allocated < 0.9 * num_slots && count < config_->num_rows_) {
  // while (allocated < num_slots && count < config_->num_rows_) {
...
void YCSBWorkload::WarmupCache() {
  ...
  if (g_buf_type == NOBUF) {
    ...
    if (g_negative_point_wl) {
      ...
      LOG_ERROR("WarmupPageNegativeScan is not supported for PGBUF");
      return;
    } else {
      return;
    }
