...
if __name__ == "__main__":
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    exp_name = "negativepoint_tuple_varydomain_015buffer_skew_debug"
    config = {
    ...
    exp_config = {
        "idx_": [0],
        "domain_size_": [1024000*20],
        "dataload_domain_size_": [1024000*20],
        "num_rows_": [1024000],
    }
    ...
    for idx in exp_config["idx_"]:
        ...
        for negative_optimization in [1, 0]:
            exps.append({"domain_size": exp_config["domain_size_"][idx], "dataload_domain_size": exp_config["dataload_domain_size_"][idx], "num_rows": exp_config["num_rows_"][idx], "negative_optimization": negative_optimization})
     # set cosmos capacity at 4000 

    for exp in exps:     

        if exp["negative_optimization"] == 0:
        ...
        # working_set = db_config["g_record_size"] * ycsb_config["domain_size_"]
        working_set = db_config["g_record_size"] * ycsb_config["num_rows_"]
        db_config["g_total_buf_sz"] = round(working_set * 0.3)
