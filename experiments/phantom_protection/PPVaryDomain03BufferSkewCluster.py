...
if __name__ == "__main__":
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    exp_name = "pp_varydomain_03buffer_skewcluster_debug"
    config = {
                ...
                for zipf in [1.5]:
                    for record_sz in [1024]:
                        for negative_scan_opt in [0]:
                            ...
                            # for phantom_type in [0]:
                                if phantom_type == 2:
                                    for partition_sz in [500]:
                                        for domain_size_scale in [1]:
                                    # for partition_sz in [100000, 10000, 1000, 100, 10]:
                                    #     for domain_size_scale in [1, 50, 100, 200, 400]:
                                    # for partition_sz in [100000, 1000, 100, 10]:
                                        # for domain_size_scale in [80, 40, 20, 10, 1]:
                                            exps.append([buf_sz, read_perc, zipf, queue, record_sz, negative_scan_opt, phantom_type, partition_sz, domain_size_scale * basic_domain_size])
                                else:
                                    # for domain_size_scale in [1, 25, 50, 100, 200, 400]:
                                    for domain_size_scale in [1]:
                                        exps.append([buf_sz, read_perc, zipf, queue, record_sz, negative_scan_opt, phantom_type, 0, domain_size_scale * basic_domain_size])
        break
# set cosmos capacity at 32000 
    for exp in exps:
        ...
        ycsb_config["dataload_domain_size_"] = exp[8]
        ycsb_config["domain_size_"] = exp[8] 


        working_set = db_config["g_record_size"] * ycsb_config["num_rows_"]
        # working_set = db_config["g_record_size"] * ycsb_config["domain_size_"]
        if exp[6] == 2:
            db_config["g_total_buf_sz"] = round(working_set * exp[0] - ((exp[8]/exp[7]) * 12))
            # print("XXX Total buf sz for PCL: ", db_config["g_total_buf_sz"])
        else:
            db_config["g_total_buf_sz"] = round(working_set * exp[0])
            # print("XXX Total buf sz for non-PCL: ", db_config["g_total_buf_sz"])
        ...
        trial_id = "azure16k-tuple-buf{}-zipf{}-phantom{}-domain{}g-ps{}".format(exp[0], exp[2], exp[6], exp[8]/1024000, exp[7])
