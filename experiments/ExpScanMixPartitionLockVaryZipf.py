...
if __name__ == "__main__":
    ...
    exp_name = "exp_mix_scan_partition_lock_vary_zipf"
    config = {
    ...
    db_config = {
        ...
        # "g_scan_zipf_random_hotspots": True,
    }
    ycsb_config = {
        "num_rows_": 10240000,
        "runtime_": 60,
    ...
    # for buf_sz in [1.25]:
    for buf_sz in [1.25]:
    #   for pt_sz in [10000, 1000]:
    #   for pt_sz in [1000, 10000, 50000]:
      for zipf in [0.99, 0.8, 0.6, 0.4, 0.2]:
        exps.append([buf_sz, zipf])

    for exp in exps:
        db_config["g_total_buf_sz"] = round(working_set * exp[0])
        db_config["g_partition_covering_lock_unit_sz"] = 10000
        db_config["g_num_worker_threads"] = 16
        ycsb_config["zipf_theta_"] = exp[1]

        trial_id = "azure16k-tuple-buf{}-zipf{}".format(exp[0], exp[1])
