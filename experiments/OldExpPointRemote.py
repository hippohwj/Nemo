...
if __name__ == "__main__":
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    exp_name = "debug_exp_point_10g_remote"
    config = {
    ...
    for queue in [100]:
        ...
        # no buffer case has no need to change buf size
        for buf_sz in [0.1]:
        ...
        break
# set cosmos capacity at 20000 when read_prec = 0.5 and buf_sz = 0.1 for remote baseline
    for exp in exps:
        ...
        trial_id = "azure16k-no-buf{}-read{}-zipf{}".format(exp[0], exp[1], exp[2])
