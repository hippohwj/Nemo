...
if __name__ == "__main__":
    ...
    for queue in [100]:
        ...
        # for buf_sz in [0.25, 0.5, 1, 1.25]:
        for buf_sz in [1.25]:
        # for buf_sz in [0.1, 0.25, 0.5, 1.25]:
            # for read_perc in [1, 0.9, 0.5]:
            # for read_perc in [1, 0.9]:
            # for read_perc in [1]:
            for read_perc in [0.5, 1]:
                # for zipf in [0.3, 0.5, 0.7, 0.9]:
                for zipf in [0.9, 0.7, 0.5, 0.3]:
                # for zipf in [0.5, 0.3]:
                # for zipf in [0.9]:
                    exps.append([buf_sz, read_perc, zipf, queue])
        break
# set cosmos capacity at 20000 when read_prec = 0.5 and buf_sz = 0.1
    for exp in exps:
