...
if __name__ == "__main__":
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    exp_name = "exp_point_100m_record_period"
    config = {
    ...
    for queue in [100]:
        ...
        # for buf_sz in [0.01, 0.1, 0.25, 0.5, 1.25]:
            ...
            for read_perc in [1]:
                ...
                # for zipf in [0.99, 0.8, 0.7]:
                # for zipf in [0.9]:
                for zipf in [0]:
                    for period in [0, 5, 10, 15, 20]:
                        exps.append([buf_sz, read_perc, zipf, queue, period])
        break
# set cosmos capacity at 20000 when read_prec = 0.5 and buf_sz = 0.1 for page baseline
    for exp in exps:
        ...
        db_config["g_batch_eviction_period_ms"] = exp[4]
        working_set = db_config["g_record_size"] * ycsb_config["num_rows_"]
        db_config["g_total_buf_sz"] = round(working_set * exp[0])

        trial_id = "azure16k-tuple-buf{}-read{}-zipf{}-period{}".format(exp[0], exp[1], exp[2], exp[4])
