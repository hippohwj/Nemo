...
if __name__ == "__main__":
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    exp_name = "exp_negative_range_cluster_large_domain_100m_vary_scanlen"
    config = {
    ...
    for queue in [100]:
        ...
        for buf_sz in [1]:
        # for buf_sz in [0.01, 0.1, 0.25, 0.5, 1.25]:
            ...
            for read_perc in [1]:
                ...
                # for zipf in [0.99, 0.8, 0.6, 0.4, 0.2]:
                for zipf in [0.99]:
                    for record_sz in [1024]:
                        for negative_scan_opt in [0]:
                            for scan_len in [10, 20, 50, 100, 200, 500, 1000]:
                                exps.append([buf_sz, read_perc, zipf, queue, record_sz, negative_scan_opt, scan_len])
        break
...
# set cosmos capacity at 20000 when read_prec = 0.5 and buf_sz = 0.1 for page baseline
    for exp in exps:
        ...
        output_filename = out_dir + "{}-recordsz{}-buf{}-read{}-negative{}.txt".format(exp_name, exp[4], exp[0], exp[1], exp[5]) 
        print("Output Filename: " + output_filename, flush=True)
        db_config["g_out_fname"] = output_filename


        trial_id = "azure16k-tuple-buf{}-read{}-zipf{}-recordsz{}-negative{}-scanlen{}".format(exp[0], exp[1], exp[2], exp[4], exp[5], exp[6])
